...
const suggestOutfitStylesPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert AI fashion stylist. Your single most important task is to provide outfit recommendations that are a perfect match for the user's uploaded clothing item, tailored for the specified gender.
  ...
  **Your Task:**
  Generate three distinct, sophisticated, and modern outfit styles that **perfectly incorporate and complement this specific item for a {{{gender}}}.** Do not suggest anything that would not look good with the provided item. All your recommendations must be based on the uploaded item and the specified gender.
  ...
  2.  **Provide Real Shopping Links**: For each recommended item, you must provide a valid, clickable shopping link from a major online fashion retailer (e.g., Amazon Fashion, Myntra, Zara, H&M, ASOS) that is **specifically for the {{{gender}}}'s version of that item.** This is critical. A link for a woman's item is incorrect if the gender is man.
  ...
  Your output must be a JSON object that strictly follows the provided schema. Your suggestions must be awesome, well-reasoned, and directly relevant to the user's item and gender.
  `,
});
...
const suggestOutfitStylesFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const styleGuidance = await suggestOutfitStylesPrompt({ ...analysis, gender: input.gender });
    ...
    const imagePromises = styleGuidance.output.outfitSuggestions.map(async (suggestion) => {
      try {
        ...
        const imageResult = await generateAIStyledOutfitImage({
          ...
          styleDescription: `A full-body, realistic, high-fashion photograph of a ${input.gender} wearing a complete, stylish ${suggestion.styleName} outfit. ${suggestion.description}`,
          gender: input.gender,
        });
